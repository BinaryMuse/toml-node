function compile(nodes) {
  ...
  function assign(node) {
    ...
    if (!pathOrSubpathAssigned(fullPath)) {
      assignedPaths.push(fullPath);
    }
  }


  function pathOrSubpathAssigned(path) {
  ...
  function deepRef(start, path, value, line, column) {
    ...
    for (var i in keys) {
      ...
      } else if (i !== keysLen - 1 && typeof ctx[key] !== "object") {
        // already a non-object value at key, can't be used as part of a new path
        genError("Cannot redefine existing key '" + traversed.join(".") + "'.", line, column);
      }
