...
function compile(nodes) {
  ...
  function reduceInlineTableNode(values) {
    ...
    for (var i = 0; i < values.length; i++) {
      ...
      var keys = val.key;
      // Support legacy single-string keys and new array-of-keys format
      if (!Array.isArray(keys)) keys = [keys];

      var reduced = reduceValueNode(val.value);
      setNestedKey(obj, keys, reduced, val.line, val.column);
    }

    return obj;
  }

  function setNestedKey(obj, keys, value, line, column) {
    for (var i = 0; i < keys.length - 1; i++) {
      var k = keys[i];
      if (typeof obj[k] === "undefined") {
        obj[k] = Object.create(null);
      } else if (typeof obj[k] !== "object" || obj[k] === null || Array.isArray(obj[k])) {
        genError("Cannot redefine existing key '" + keys.slice(0, i + 1).join(".") + "'.", line, column);
      }
      obj = obj[k];
    }
    var lastKey = keys[keys.length - 1];
