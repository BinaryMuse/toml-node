  ...
  var result = {
    ...
    parse: function(input, startRule) {
      var parseFunctions = {
        ...
        "DIGIT": parse_DIGIT,
        "DIGITS": parse_DIGITS,
        "ARY_START": parse_ARY_START,
        "ARY_END": parse_ARY_END,
        "ARY_SEP": parse_ARY_SEP,
      ...
      function parse_start() {
        ...
        if (result0 !== null) {
          result0 = (function(offset, line, column) { return reduce(nodes) })(pos0.offset, pos0.line, pos0.column);
        }
        if (result0 === null) {
          pos = clone(pos0);
        }
      ...
      function parse_ARY_START() {
        var cacheKey = "ARY_START@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_ARY_END() {
        var cacheKey = "ARY_END@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function parse_ARY_SEP() {
        var cacheKey = "ARY_SEP@" + pos.offset;
        var cachedResult = cache[cacheKey];
        if (cachedResult) {
          pos = clone(cachedResult.nextPos);
          return cachedResult.result;
      ...
      function cleanupExpected(expected) {
          ...
          for(i in nodes) {
            ...
            switch(node.type) {
              case 'Set':
                ...
                resolveValue(node.line, node.column, node.key, value);
                break;
              case 'Group':
                resolveGroup(node.line, node.column, node.value);
                break;
            }
          }
        };
      
        function resolveValue(line, col, key, value) {
        ...
        function resolveGroup(line, col, keygroup) {
        ...
        // Reduce the nodes into a standard JavaScript object
        // function reduceold() {
        //   for(i in nodes) {
        //     var node = nodes[i];
        //     switch(node.type) {
        ...
        //         var value = node.value.value
        //         if (node.value.type == 'Array') value = processArray(value);
        //         if (pathRef[node.key] !== undefined) {
        //           var path = (currentPath ? currentPath + '.' + node.key : node.key);
        //           var error = "Error: cannot redefine key" + path +
        //             " at line " + node.line + ", column " + node.column;
        //           throw new Error(error);
        //         } else {
        //           pathRef[node.key] = value;
        //         }
        //         break;
      
        //       case 'Group':
        //         var existingValue =
        //         if (definedPaths.indexOf(node.value) == -1) {
        //           definedPaths.push(node.value);
        //           currentPath = node.value;
        //           pathRef = deepValue(currentPath);
        //         } else {
        //           var error = "Error: cannot redefine keygroup" + node.value +
        //             " at line " + node.line + ", column " + node.column;
        //           throw new Error(error);
        //         }
        //         break;
